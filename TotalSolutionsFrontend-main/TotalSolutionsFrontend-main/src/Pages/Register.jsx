...
const Register = () => {
    ...
    const fetchExistingData = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          const mobileNumbers = response.data.users.map((user) => user.mobilenumber);
  ...
  const validateForm = () => {
    ...
    } else if (!mobileRegex.test(formData.mobilenumber.toString())) {
      errors.mobilenumber = "Please enter a valid 10-digit mobile number";
    } else if (existingMobileNumbers.includes(formData.mobilenumber)) {
