  ...
  useEffect(() => {
    const fetchChildren = async () => {
      try {
        ...
        // Client-side adjustment for admitStatus based on staff assignment
        const adjustedChildren = response.data.map((child) => {
          const hasTherapies =
            Array.isArray(child.therapies) && child.therapies.length > 0;

          return child;
        });

        setChildren(adjustedChildren);
        setFilteredChildren(adjustedChildren);
        setLoading(false);
      } catch (err) {
  ...
  const filterChildren = () => {
    ...
    if (statusFilter !== "all") {
      result = result.filter(
        (child) =>
          child.admitStatus === statusFilter ||
          child.admitStatus.startsWith(statusFilter),
      );
    }
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        onClick={(e) => handleReports(e, child)}
                      >
                        View IEP Reports
                      </button>
                    </td>
