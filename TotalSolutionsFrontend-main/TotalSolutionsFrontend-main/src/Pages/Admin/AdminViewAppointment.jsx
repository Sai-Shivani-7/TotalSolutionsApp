...
export default function AppointmentsTable() {
  ...
  const handleDateSelect = async () => {
    ...
    try {
      ...
      // Fetch slot configuration and booked slots in parallel
      const [slotsResponse, bookedResponse] = await Promise.all([
        axios.get(
          `/api/superadmin/slot-config/${currentAppointment.doctorId}?date=${editDate}`,
          {
      ...
      const slotConfig = slotsResponse.data[0]; // superadmin endpoint returns array
      const bookedData = bookedResponse.data;

      // Check if it's a holiday
      if (slotConfig && slotConfig.isHoliday) {
        setAvailableSlots([]);
        setEditTime('');
        setEditStep(2);
        // Show a toast message for holiday
        toast.info(`Cannot reschedule - ${slotConfig.holidayName} is a holiday`);
        return;
      }

      // Extract available slots from configuration
      let configuredSlots = [];
      if (slotConfig && slotConfig.availableSlots && slotConfig.availableSlots.length > 0) {
        configuredSlots = slotConfig.availableSlots
          .filter(slot => slot.isActive && !slot.isBlocked)
          .map(slot => slot.time);
      } else {
        // Fallback to default slots if no configuration exists
        configuredSlots = DEFAULT_TIME_SLOTS;
      }

      // Filter out already booked slots
      const unavailableSlots = bookedData.unavailableSlots || [];
      const availableSlots = configuredSlots.filter(slot => !unavailableSlots.includes(slot));
      
      setAvailableSlots(availableSlots);

      // If current time is still available, keep it selected
      if (availableSlots.includes(currentAppointment.appointmentTime)) {
        setEditTime(currentAppointment.appointmentTime);
      } else {
        setEditTime(availableSlots[0] || '');
      }
  ...
  const handleRescheduleAppointment = async () => {
    ...
    try {
      ...
      if (response.data.success !== false) {
        ...
        setUnprocessedData(refreshResponse.data);
      } else {
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl">
              ...
              <div>
                <div className="mb-4">
                  <p className="text-sm text-gray-600 mb-2">
                    <strong>Current appointment:</strong> {currentAppointment.appointmentDateFormatted} at {currentAppointment.appointmentTime}
                  </p>
                  <p className="text-sm text-gray-600 mb-4">
                    <strong>Child:</strong> {currentAppointment.childName}
                  </p>
                </div>
                
                <div className="mb-6">
                  <label htmlFor="editDate" className="block text-sm font-medium text-[#ab1c1c] mb-2">
                    Select New Date
                  </label>
                  <input
                    type="date"
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button
                    onClick={handleDateSelect}
                    disabled={!editDate || loadingSlots}
                    className={`px-4 py-2 rounded-lg font-medium ${
                      !editDate || loadingSlots
                        ? 'bg-gray-300 cursor-not-allowed'
                        : 'bg-[#ab1c1c] text-white hover:bg-[#8a1717]'
                    }`}
                  >
                    {loadingSlots ? 'Loading...' : 'Next'}
                  </button>
                </div>
              </div>
            )}
            ...
            {editStep === 2 && (
              <div>
                <div className="mb-4">
                  <p className="text-sm text-gray-600 mb-4">
                    <strong>Selected Date:</strong> {new Date(editDate).toLocaleDateString('en-GB')}
                  </p>
                </div>

                <div className="mb-6">
                  <label className="block text-sm font-medium text-[#ab1c1c] mb-2">
                      ...
                      {availableSlots.map((slot) => (
                        <button
                          ...
                          className={`p-2 text-sm border rounded-lg ${
                            editTime === slot
                              ? 'bg-[#ab1c1c] text-white border-[#ab1c1c]'
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  <button
                    ...
                    disabled={!editTime || loadingSlots}
                    className={`px-4 py-2 rounded-lg font-medium ${
                      !editTime || loadingSlots
                        ? 'bg-gray-300 cursor-not-allowed'
                        : 'bg-[#ab1c1c] text-white hover:bg-[#8a1717]'
                    }`}
                  >
                    {loadingSlots ? 'Rescheduling...' : 'Reschedule Appointment'}
                  </button>
                </div>
              </div>
            )}
