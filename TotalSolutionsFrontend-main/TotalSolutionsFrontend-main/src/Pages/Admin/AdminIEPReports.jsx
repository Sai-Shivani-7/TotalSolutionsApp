...
const API_BASE = import.meta.env.VITE_API_URL || "https://totalapi.joywithlearning.com";
...
export default function AdminIEPReports() {
  ...
  useEffect(() => {
  const fetchIEPReports = async () => {
    try {
      const response = await axios.get(`/api/admins/child/${childId}`, {
        headers: {
          Authorization: sessionStorage.getItem("token"),
        },
      });

      setChildName(response.data.name || "Child");

      const filteredIEP =
        response.data.reports?.filter(
          (report) => report.type === "IEP"
        ) || [];
        console.log("ieps are : ",filteredIEP);
      setIepReports(filteredIEP);
    } catch (error) {
      console.log("Error fetching IEP reports:", error);
    }
  };
...
const handleViewIEPReport = (filePath) => {
...
const formatDate = (date) =>
  ...
  return (
    <div className="container mx-auto px-4 py-6 mt-4 flex flex-col items-center justify-center ">
      ...
      {/* ----------- PAST IEP REPORTS (NEW) ----------- */}
<div className="bg-white shadow-lg rounded-lg p-4 sm:p-6 w-full max-w-4xl mt-10">
  <h2 className="text-xl font-semibold text-[#ab1c1c] mb-4">
    Past IEP Reports {childName && `– ${childName}`}
  </h2>
        ...
        {iepReports.map((report) => (
          <tr key={report._id} className="border-t hover:bg-gray-50">
            ...
            <td className="px-4 py-2">
              <button
                onClick={() => handleViewIEPReport(report.filePath)}
