...
export default function IEPDoctor() {
  ...
  useEffect(() => {
    if (selectedMonthDetails?.childVideo?.videoUrl) {
      const fetchVideo = async () => {
        try {
          const encodedUrl = encodeURIComponent(selectedMonthDetails.childVideo.videoUrl);
      ...
      return () => {
        if (displayVideo) {
          URL.revokeObjectURL(displayVideo);
        }
      };
    }
